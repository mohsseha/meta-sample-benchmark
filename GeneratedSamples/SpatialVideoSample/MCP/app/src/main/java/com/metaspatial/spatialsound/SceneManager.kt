...
class SceneManager(private val systemManager: SystemManager) : ISystem, SurfaceHolder.Callback {
    ...
    private fun createVideoPanel() {
        ...
        val layoutId = context.resources.getIdentifier("video_panel_layout", "layout", context.packageName)
        val panelLayout = inflater.inflate(layoutId, null) as FrameLayout
        val surfaceViewId = context.resources.getIdentifier("surface_view", "id", context.packageName)
        surfaceView = panelLayout.findViewById(surfaceViewId)
